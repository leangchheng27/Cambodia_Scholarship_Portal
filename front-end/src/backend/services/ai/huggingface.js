 ...
 */
export async function getAIRecommendations(userProfile, scholarships, limit = AI_CONFIG.DEFAULT_LIMIT) {
  try {
    console.log('getAIRecommendations - Called with userProfile:', userProfile);
    console.log('getAIRecommendations - User grades:', userProfile?.grades);
    console.log('getAIRecommendations - User studentType:', userProfile?.studentType);
    console.log('getAIRecommendations - User GPA:', userProfile?.gpa);
    
    // Generate user profile embedding
    const userText = getUserProfileEmbeddingText(userProfile);
    console.log('getAIRecommendations - User profile text for embedding:', userText);
    
    const userEmbedding = await generateEmbedding(userText);
    
    if (!userEmbedding) {
      console.log('getAIRecommendations - No AI embeddings, falling back to rule-based matching');
    }
    ...
    const scoredScholarships = await Promise.all(
      scholarships.map(async (scholarship) => {
        ...
        if (userEmbedding) {
          ...
          const scholarshipEmbedding = await generateEmbedding(scholarshipText);
        ...
        // If AI embedding failed for this scholarship or no API key, use rule-based matching
        if (!usedAI) {
    ...
    console.log('getAIRecommendations - Sorted scholarships:');
    scoredScholarships.slice(0, limit).forEach((s, i) => {
 ...
 */
export async function precomputeScholarshipEmbeddings(scholarships) {
  ...
  localStorage.setItem(AI_CONFIG.CACHE_KEY, JSON.stringify(embeddingsCache));
...
/**
 * Get cached scholarship embeddings from localStorage
 * @returns {Object} - Cached embeddings or empty object
 */
export function getCachedEmbeddings() {
  const cached = localStorage.getItem(AI_CONFIG.CACHE_KEY);
...
export function clearEmbeddingsCache() {
  localStorage.removeItem(AI_CONFIG.CACHE_KEY);
