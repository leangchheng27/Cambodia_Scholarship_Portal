...
export function calculateMatchScore(userProfile, scholarship) {
  console.log('calculateMatchScore - Called for scholarship:', scholarship.title);
  console.log('calculateMatchScore - User profile:', { studentType: userProfile.studentType, grades: userProfile.grades, gpa: userProfile.gpa });
  ...
  if (scholarship.aiMetadata?.studentTypes) {
    if (scholarship.aiMetadata.studentTypes.includes(userProfile.studentType) || 
      ...
      console.log(`  Student type match: +${MATCH_WEIGHTS.STUDENT_TYPE} points`);
    }
  } else {
    score += MATCH_WEIGHTS.STUDENT_TYPE / 2; // Partial credit if not specified
    console.log(`  Student type not specified: +${MATCH_WEIGHTS.STUDENT_TYPE / 2} points`);
  }
  ...
  if (scholarship.aiMetadata?.fieldCategories) {
    ...
    const fieldScore = Math.min(MATCH_WEIGHTS.FIELD_MATCH, matchingFields.length * 10);
    score += fieldScore;
    console.log(`  Field match: ${matchingFields.length} fields, +${fieldScore} points`);
  }
  ...
  if (scholarship.aiMetadata?.minGPA) {
    if (gpa >= scholarship.aiMetadata.minGPA) {
      ...
      console.log(`  GPA meets requirement (${scholarship.aiMetadata.minGPA}): +${MATCH_WEIGHTS.GPA_REQUIREMENT} points`);
    } else if (gpa >= scholarship.aiMetadata.minGPA - 0.5) {
      score += MATCH_WEIGHTS.GPA_REQUIREMENT / 2; // Close to requirement
      console.log(`  GPA close to requirement: +${MATCH_WEIGHTS.GPA_REQUIREMENT / 2} points`);
    }
  } else {
    score += MATCH_WEIGHTS.GPA_REQUIREMENT * 0.75; // Partial credit if not specified
    console.log(`  GPA requirement not specified: +${MATCH_WEIGHTS.GPA_REQUIREMENT * 0.75} points`);
  }
  ...
  if (scholarship.aiMetadata?.requiredSubjects) {
    ...
    const subjectScore = Math.round(MATCH_WEIGHTS.REQUIRED_SUBJECTS * subjectMatchRatio);
    score += subjectScore;
    console.log(`  Required subjects match: ${matchingSubjects.length}/${scholarship.aiMetadata.requiredSubjects.length}, +${subjectScore} points`);
  } else {
    score += MATCH_WEIGHTS.REQUIRED_SUBJECTS / 2; // Partial credit if not specified
    console.log(`  Required subjects not specified: +${MATCH_WEIGHTS.REQUIRED_SUBJECTS / 2} points`);
  }
  
  const finalScore = Math.min(maxScore, Math.round(score));
  console.log(`calculateMatchScore - Final score for ${scholarship.title}: ${finalScore}`);
  return finalScore;
}
 ...
 */
export function generateMatchReasons(userProfile, scholarship) {
  const reasons = [];
  const recommendedFields = getRecommendedFields(userProfile.studentType, userProfile.grades);
